...
public class MyPlane extends GameObject {
	...
	public void initBitmap() {
		...
		object_width = myplane.getWidth() / 2; // ??????ฆห????
		object_height = myplane.getHeight(); // ??????ฆห????
	}
	...
	public void drawSelf(Canvas canvas) {
		// TODO Auto-generated method stub
		if(isAlive){
			int x = (int) (currentFrame * object_width); // ???????????ฆห???Y????
			canvas.save();
						...
						if(obj.isCollide(pobj)){			//??????
							...
							if(pobj.isExplosion()){	
								...
								if(pobj instanceof SmallPlane){
									mainView.getSounds().playSound(2, 0);		//?????????งน
								}
								else if(pobj instanceof MiddlePlane){
									mainView.getSounds().playSound(3, 0);		//?????????งน
								}
								else{
									mainView.getSounds().playSound(4, 0);		//?????????งน
								}			
							}
							break;
						}
